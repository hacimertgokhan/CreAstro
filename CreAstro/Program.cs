...
void Start() {
    var gitrecre = true; 
    LoggingTools.Send("For the repo creation, you must log-in your github account.");
    ...
    while (gitrecre)
    {
        LoggingTools.Empty(1);
        Console.Write(" >> ");
        var repname = Console.ReadLine();
        if(!(repname.Equals("help") | repname.Equals("exit") | repname.Equals("me") | repname.Equals("developer"))) Console.WriteLine($"There is no subcommand like '{repname}'");
        switch (repname)
        {
            case "help":
            {
